        ...
        for e in range(self.n_expert):
            ...
            for slot in range(self.k):
                ...
                if sel.any():
                    ...
                    #
                    #   3. y[sel] += ...：
                    #      y[sel] 同样用 sel 索引选出对应行，然后 += 累加
                    #      为什么用 += 而非 =？
                    #        同一个 token 可能在不同的 slot 上（不同循环迭代）被分配了
                    #        同一个专家（虽然 top-k 保证了 k 个 slot 的专家编号互不相同，
                    #        但这里用的是"slot"级别的分发，不是 token 级别的去重）。
                    #        更关键的是：同一个 token 的 k 个不同专家输出会分别通过
                    #        y[sel] += ... 累加到同一行中，最终形成加权和。
                    #        这正是 "MoE = Σ_k w_k × Expert_k(x)" 公式在代码中的体现。
                    #
                    #   直观理解：y 是一个"签名簿"——
                    #     每个 token（行）有一个签名区（C 维的向量）。
                    #     每个专家看过 token 后在对应位置写下评语，前面的评语不会被覆盖，
                    #     新评语叠加上去，最终每个 token 携带的是 k 个专家的"加权综合意见"。
